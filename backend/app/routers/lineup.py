...
@router.post("/apply")
async def apply_recommended_lineup_endpoint(req: ApplyLineupRequest):
    """Preview or apply the recommended lineup to ESPN.

    confirm=false always returns a preview of the moves without sending
    anything. confirm=true submits the transaction to ESPN — but only when
    ESPN_WRITE_ENABLED is set; otherwise the exact payload is logged for
    validation and nothing is sent (dry run).
    """
    from ..jobs.apply_lineup import apply_recommended_lineup

    try:
        return await apply_recommended_lineup(req.confirm)
